...
class BC(object):
    def __init__(self, download_path=None, sigma=0.3, order=7,
        ...
        self.mmae = np.load(
            os.path.join(download_path, "mean_vel_smplxflame_30.npy")) if download_path is not None else None
        self.threshold = 0.10
        self.counter = 0
        self.sum_a2m = 0
        self.sum_m2a = 0
    ...
    def load_motion(self, pose, t_start, t_end, pose_fps, without_file=False):
        ...
        vel = np.concatenate([init_vel, middle_vel, final_vel], 1).transpose(1, 0).reshape(data_each_file.shape[0], -1,
                                                                                           3)
        # print(vel.shape)
    ...
    def avg(self):
        return {'a2m': self.sum_a2m / self.counter, 'm2a': self.sum_m2a / self.counter}
